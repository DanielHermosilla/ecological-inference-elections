...
eim <- R6::R6Class("eim",
    public = list(
        ...
        initialize = function(X = NULL, W = NULL, json_path = NULL) {
			...
			# ===== DIRECT MATRIX READING ==== #
				...
				# Validations
                if (is.null(X) || is.null(W)) stop("Either provide X and W or an valid JSON path")
				private$validate_X_W(data$X, data$W)	
				# Update parameters
                self$X <- as.matrix(X)
                self$W <- as.matrix(W)
        ...
        #' @description Shows, in form of a list, a selection of the most important atributes. It'll retrieve the method, amount of candidates, ballots and groups and the principal resuls of the EM algorithm.
    ...
    active = list(
        ...
        #' @field multivariate_iterations (numeric(1)) Active variable to show the error threshold for the Montecarlo simulation of the Multivariate Normal CDF
    ...
    private = list(
	   ...
	   # PRIVATE FIELD mvn_iterations The number of iterations for the MVN CDF Monte Carlo simulation.
		...
		# PRIVATE METHOD validates an X or W matrix
...
#'
#' 		\item \var{verbose} (boolean(1)) Boolean indicating wether to print useful messages while iterating. (default: FALSE). \cr
#'
#'     \item \strong{"Hit and Run" Method:}
#'       \itemize{
