...
test_that("Computation works", {
    for (i in 1:tests) {
        ...
        if (candidates < 4 && groups < 4) {
            ...
            expect_condition(is.integer(model$total_iterations) == TRUE)
            expect_condition(is.matrix(model$probability) == TRUE)
            expect_condition((is.numeric(x) && is.atomic(x)) == TRUE)
        }
