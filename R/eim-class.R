...
#'
#' @param symmetric_weight_method Character string indicating how to combine both directions when `symmetric = TRUE`. Valid values are `"joint"` (default behavior: joint EM where forward/reverse E-steps are averaged each iteration using equal weights), `"average"` (classic two-run average), `"delta_ll"` (weights based on the relative likelihood gaps with `alpha = tau / (tau + tau_rev)`, where `tau` is the regular-direction relative gap and `tau_rev` is the reverse-direction relative gap), and `"mae_inverse"` (weights based on reconstruction errors with `alpha = nu_rev / (nu + nu_rev)`). For `"delta_ll"` and `"mae_inverse"`, non-positive directions are clipped with `max(value, .Machine$double.eps)` before computing weights. In parametric mode, `"joint"` and `"average"` are supported directly; other values keep the classic equal-average fallback.
#'
