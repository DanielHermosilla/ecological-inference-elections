...
test_that(".validate_json_eim:\tRejects incorrect json paths.", {
    expect_error(eimodel:::.validate_json_eim("invalid/path.json"))
    expect_error(eimodel:::.validate_json_eim("invalid/file"))
    expect_error(eimodel:::.validate_json_eim("~/"))
    expect_error(eimodel:::.validate_json_eim(daniel))
})
...
test_that(".validate_eim:\tValid inputs return TRUE", {
    ...
    expect_true(eimodel:::.validate_eim(X, W))
})

test_that(".validate_eim:\tError if X is NULL", {
    W <- matrix(13:24, nrow = 3, ncol = 4)
    expect_error(
        eimodel:::.validate_eim(NULL, W),
...
test_that(".validate_eim:\tPasses on row mismatch when mismatch = TRUE", {
    X <- matrix(1:8, nrow = 2, ncol = 4)
    W <- matrix(9:20, nrow = 3, ncol = 4)
    expect_true(eimodel:::.validate_eim(X, W, mismatch = TRUE))
})
