...
#include "utils_matrix.h"
     ...
     *
     * Given a pointer to an array of matrices, it writes them to a binary file. Usually it's useful for saving the `X`
     * and `W` matrices with their probabilities matrix. It's also possible to save a single matrix.
    ...
    void writeResultsJSON(const char *outputFileName, const char *inputFileName, const char *methodUsed,
                          const double convergence, const double iterations, const double time,
                          const int iterationsMade, const Matrix *pReal, const Matrix *pCalculated, const double *logLL,
                          int S, int M, bool hit);
#ifdef __cplusplus
}
#endif
#endif
