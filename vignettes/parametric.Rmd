...
An election can be simulated using the function `simulate_election()`. When employing the parametric approach, it is necessary to specify both the number of covariates (`num_attributes`) and the number of districts (`num_districts`). In practice, a district may represent a collection of ballot boxes that share the same covariate values. In the example below, we simulate an election with three candidates, two demographic groups, and 20 covariates.
...
Note that the parametric setting must be explicitly enabled by specifying parametric = TRUE.
